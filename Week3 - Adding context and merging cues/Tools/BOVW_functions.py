...
def getKeypointsDescriptors(filenames,detector_type,descriptor_type):
    ...
    if not (descriptor_type == 'HOG' or descriptor_type == 'LBP'or descriptor_type == 'color'):
...
def computePCA(data,experimentType, pca_ncomponents, my_model):
    ...
    #                                           2 = performs a PCA with n_components and a given model
    init=time.time()
    #data = [normalize(x) for x in data]# You need to normalize your data first
    if experimentType == 1:
        ...
        plt.show()
    elif experimentType == 0:
        print 'Computing PCA using '+str(pca_ncomponents)+' components.'
        my_model = PCA(pca_ncomponents)
        data = my_model.fit_transform(data)

        #for x in data:
        #    my_model.fit(x)
        #data = [my_model.transform(x) for x in data]
        end=time.time()
    else:
        print 'Computing PCA using '+str(pca_ncomponents)+' components.'
        data = my_model.fit_transform(data)
    ...
    return data, my_model # Return the eigenvalues
...
def getAndSaveBoVW_SPMRepresentation_GMM(descriptors,keypoints,k,gmm,filename,files):
...
def trainAndTestLinearSVM_withfolds(train,test,GT_train,GT_test,folds,start,end,numparams):
    ...
    tuned_parameters = [{'kernel': ['rbf'], 'gamma': [1e-3, 1e-4],
                     'C': [1, 10, 100, 1000]},
                    {'kernel': ['linear'], 'C':np.linspace(start,end,num=numparams)}]
    clf = GridSearchCV(svm.SVC(decision_function_shape='ovr'), tuned_parameters, cv=folds,scoring='accuracy',n_jobs = 6)
...
def trainAndTestHISVM_withfolds(train,test,GT_train,GT_test,folds):
    ...
    clf = GridSearchCV(svm.SVC(), tuned_parameters, cv=folds,scoring='accuracy',n_jobs = 6)
    clf.fit(kernelMatrix, GT_train)
    print(clf.best_params_)
    predictMatrix = histogramIntersection(stdSlr.transform(test), train)
    SVMpredictions = clf.predict(predictMatrix)
