...
# #
def getFeatList(FileList, params, runmultiprocess):
    ...
    for imfile in FileList:
        ...
        if runmultiprocess:         ########### start change
            ...
            feats, im_idx = getFeatImgSample(img,params,n_im,True,False);
            if shape(feats)[0]>0:
                idxs[im_idx,0]=idx
...
def MeanAveragePrecision(filename, trainDir, LIBRARY, idxs, k):
...
def showQueryResult(filename,trainDir,LIBRARY,idxs,method):
    ...
    #                    
    plt.ion()
    plt.figure('IMAGE RETRIEVAL'); plt.subplot(353); plt.title('QUERY')
    ...
    for i in range(0,10):
        ...
        try:
            ...
            print LIBRARY[idxs[i]].split('/')[-1][3:-4]
