...
def bow_encoding(features, codebook):
    ...
    if soft_assignment:
        ...
        hist[0,:]=np.bincount(code,minlength=codebook.shape[0])
        #for i in range(len(code)):
        #    c = code[i]
        #    visual_words[i,:]=np.bincount(words,minlength=k)
        #    hist[0, c-1] += 1

    histNorm = hist/np.sum(hist)    # normalize
    #print sum(hist)
    #print len(code)
    return histNorm
...
def computeBoW(TRAINFILES,params,runmultiprocess,bowDir,showFig='False'):
    ...
    #
    X,idxs = tl.getFeatList(TRAINFILES,params,runmultiprocess)
    ...
    print params['K']
    #X = vq.whiten(X)
    centroids,variance = vq.kmeans2(X,params['K'],1)
