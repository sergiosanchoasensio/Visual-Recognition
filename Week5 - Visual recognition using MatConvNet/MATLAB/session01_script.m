...
% % Run the code
% 
% [alexNet, info] = mynet_train(imdb, 'results/mynet5')
% alexNet.print({'input',[67,67,3,1]},'MaxNumColumns', 5);

% alexNet.conserveMemory=0;
% dataset_path='data/MIT_split/';
% output_path = 'output_fc4/';
% type = {'test/','train/'};
% class = {'coast/','forest/','highway/','inside_city/','mountain/','Opencountry/','street/','tallbuilding/'};
% mkdir(output_path);
% for i = 1:size(type,2)
%     mkdir(strcat(output_path,char(type(i))));
%     for j = 1:size(class,2)
%         mkdir(strcat(output_path,char(type(i)),char(class(j))));
%         contents = dir(strcat(dataset_path,char(type(i)),char(class(j)),'*.jpg')); 
%         for k = 1:numel(contents)
%             filename = contents(k).name;
%             im = imread(strcat(dataset_path,char(type(i)),char(class(j)),filename));
%             im_ = single(im) ; % note: 0-255 range
%             im_ = imresize(im_, [67 67]); % rescale [227 227 3]
...
%             scores = alexNet.vars(alexNet.getVarIndex('fc4')).value;
%             scores = squeeze(gather(scores));
%             save(strcat(output_path,char(type(i)),char(class(j)),filename(1:end-4),'.mat'),'scores')
%         end
%     end
% end