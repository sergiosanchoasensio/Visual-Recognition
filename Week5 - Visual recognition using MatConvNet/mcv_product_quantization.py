...
def computeProductQuantization(TRAINFILES,params,showFig=False,Folder=''):
    ...
    #
    CODEBOOK = compute_codebook(np.squeeze(X),params['pq_m'],params['pq_K'],params['global'],Folder)
    ...
    for imfile in TRAINFILES:
        ...
        #qv = pq.quantize_vector(X[n_im],params['pq_m'],CODEBOOK,params['-pq_K'])
        qv = quantize_vector(X[n_im],params['pq_m'],CODEBOOK,params['pq_K'])
        f = open(Folder+'pq_'+imfile.split('/')[-1][0:-5]+'.pkl', 'wb');
        pickle.dump(qv,f)
        f.close()
...
def pq_distance(x,y,dist_mode,codebook,m,k,vec_mode='coded'):
    ...
    D = len(x[0])         # dimensionality of descriptor
    D_star = D/m    # size of subvectors
...
#    INVERTED-FILE
#
def invertedFile(x,codebook,m,k):
    print 'Computing the inverted files technique ...'
    table = zeros((k,m))
    D = shape(x)[1]
    D_star = D/m
    for i in range(m):
        ...
        code,distance = vq.vq(centroids,x_star)
        table[:,i] = distance
    return table
...
def pq_query(x,CODEBOOK,params,LIBRARY,lshDir):
    if params['inverted-file']:
        LUT = invertedFile(x,CODEBOOK,params['pq_m'],params['pq_K'])
